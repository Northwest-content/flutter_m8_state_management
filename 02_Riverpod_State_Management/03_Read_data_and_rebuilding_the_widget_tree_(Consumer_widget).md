...
Notice that, if you added a new note, it doesn't show in the screen 😢
Why is that? because in the current way we can only read values, but not rerendering the widget. The widget has to be notified of the changes. And to do that, we'll have to use **`notifyListeners()`** method inside the **`addNote`** function that is inside the `NoteProvider` class.
...
2. Since we are using the **notes** variable inside the **ListView** widget, we will use the **watch** argument, and create a **noteManager** variable inside the **builder** named argument.
...
3. After we create the **noteManager** variable, we will use it to access the data that is inside the **NoteProvider** class.

Replace the `itemCount: notes.length` to `itemCount: noteManager.notes.length`,
...
4.  We can pass the selected note object inside the **`NoteListTile`,** but the best practice is to use the **`selectedIndex`** variable that we created inside the **`NoteProvider`** class before.
...
5. Because we no longer will use the **Constructor** to pass the object to other widgets or screens, and we will use the **riverpod** package.
...
6. Then inside the **builder** named argument create the **noteManager** variable; later we will use this variable to access the **notes** & **selectedIndex** variable.
...
7. Create another variable, then name it **selectedNote**, and keep its value to **noteManager.notes[noteManager.selectedIndex]**
...
8. Then use the **selectedNote** variable, to display the **note title** inside the Text widget.
...
9. We will do the same process inside the **note_page.dart** file.
